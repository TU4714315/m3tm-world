...
export async function GET(req: Request) {
  ...
  try {
    const [userRes, reposRes] = await Promise.all([
      fetch(`https://api.github.com/users/${encodeURIComponent(username)}`, { signal: AbortSignal.timeout(15000), headers: { 'User-Agent': 'M3TM-WORLD/1.0 (+https://m3tm.world)' } }),
      fetch(`https://api.github.com/users/${encodeURIComponent(username)}/repos?sort=updated&per_page=5`, { signal: AbortSignal.timeout(15000), headers: { 'User-Agent': 'M3TM-WORLD/1.0 (+https://m3tm.world)' } })
    ]);
