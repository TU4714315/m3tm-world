...
const OP_LABEL:Record<string,string>={SCAN:'فحص',PROBE:'تحقق',READ:'قراءة',WRITE:'كتابة',NOTIFY:'إشعار',CONNECT:'اتصال',DISCONNECT:'قطع',ENUM:'استعراض',ERROR:'خطأ',SUB:'اشتراك',INFO:'معلومة',GPS:'موقع',NET:'شبكة',RSSI:'إشارة'};
const SRC_LABEL:Record<string,string>={SCANNER:'الفاحص',RECON:'الشبكة',EXPORT:'تصدير'};
    ...
    <div className={`glass-panel flex flex-col overflow-hidden pointer-events-auto shrink-0 ${full?'w-full h-full':'h-[500px] max-h-[80vh] resize-y'}`}>
      ...
      <div className="flex-shrink-0 flex items-center justify-between px-3 py-2.5 border-b border-[rgba(255,255,255,0.05)] bg-[rgba(0,0,0,0.3)] hover:bg-[var(--hover-accent)] transition-colors">
        <button onClick={()=>setExpanded(!expanded)} className="flex items-center gap-2 flex-1">
          ...
          <span className="hud-text text-[11px] text-[var(--text-primary)]">الأجهزة القريبة</span>
          <span className="gotham-tag gotham-tag--info" style={{fontSize:'9px',padding:'1px 5px'}}>{devices.length} DEVS</span>
          {scanning&&<span className="gotham-tag" style={{fontSize:'9px',padding:'1px 5px',background:'rgba(0,230,255,0.1)',color:'var(--cyan-primary)'}}>جارٍ الفحص</span>}
        </button>
        <div className="flex items-center gap-2">
          <motion.button whileTap={{scale:0.95}} onClick={scan} disabled={scanning||!btOk}
            ...
            {scanning?<BluetoothSearching className="w-3 h-3 animate-pulse"/>:<Bluetooth className="w-3 h-3"/>}{scanning?'...':'فحص'}
          </motion.button>
          ...
          <button onClick={()=>setFull(!full)} className="p-1.5 -m-0.5 rounded text-[var(--text-muted)] hover:text-[var(--text-primary)] hover:bg-white/10 transition-colors" title="ملء الشاشة">
            {full?<Minimize2 className="w-3.5 h-3.5"/>:<Maximize2 className="w-3.5 h-3.5"/>}
          </button>
          <div className="w-1.5 h-1.5 rounded-full bg-[var(--cyan-primary)] animate-m3tm-pulse"/>
      ...
      <AnimatePresence>{expanded&&(
        <motion.div initial={{height:0,opacity:0}} animate={{height:'auto',opacity:1}} exit={{height:0,opacity:0}} transition={{duration:0.2}} className="flex-1 flex flex-col overflow-hidden min-h-0">
          ...
          <div className="flex items-center px-2 py-1.5 shrink-0 gap-0.5 border-b border-[rgba(255,255,255,0.04)]" style={{background:'rgba(0,0,0,0.2)'}}>
            {([['scan','الأجهزة',Scan],['intel','الشبكة',Globe],['log','السجل',Terminal]] as const).map(([id,label,Icon])=>(
                ...
                {devices.length===0&&!scanning&&btOk&&(
                  <div className="flex flex-col items-center justify-center py-8 opacity-40">
                    ...
                    <p className="text-[10px] font-mono text-[var(--text-muted)] text-center tracking-wider">
                      اضغط <span className="text-[var(--cyan-primary)]">فحص</span> للبحث عن الأجهزة<br/>
                      <span className="text-[9px] opacity-50">جمع محلي · تحديد موقع · حفظ</span>
                    </p>
                  </div>
                )}
                {!btOk&&<div className="p-2 rounded-lg" style={{background:'rgba(255,61,61,0.04)',border:'1px solid rgba(255,61,61,0.08)'}}><span className="text-[9px] font-mono text-[#FF3D3D]">Bluetooth عبر الويب غير متاح — استخدم Chrome أو Edge على localhost</span></div>}
                    ...
                    <div className="rounded-xl overflow-hidden transition-all" style={{background:dev.connected?`linear-gradient(135deg,${dev.color}06,${dev.color}02)`:'rgba(255,255,255,0.015)',border:`1px solid ${dev.connected?dev.color+'15':'rgba(255,255,255,0.04)'}`,boxShadow:dev.connected?`0 0 16px ${dev.color}08`:'none'}}>
                      ...
                      <button className="w-full px-3 py-2.5 flex items-center gap-2.5 text-left" onClick={()=>setExDev(isE?null:dev.id)}>
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-1.5 mb-0.5">
                            ...
                            {dev.connected&&<span className="text-[9px] font-mono px-1 py-0.5 rounded bg-[var(--alert-green)]/10 text-[var(--alert-green)] tracking-widest font-bold">مباشر</span>}
                          </div>
                          {dev.probing?(
                      ...
                      <AnimatePresence>{isE&&!dev.probing&&(<motion.div initial={{height:0,opacity:0}} animate={{height:'auto',opacity:1}} exit={{height:0,opacity:0}} className="overflow-hidden"><div className="px-3 pb-3" style={{borderTop:'1px solid rgba(255,255,255,0.03)'}}>
                        ...
                        <div className="mt-2 mb-2.5 rounded-lg p-2.5" style={{background:'rgba(0,0,0,0.25)',border:'1px solid rgba(255,255,255,0.03)'}}>
                          <div className="text-[9px] font-mono text-[var(--text-muted)] tracking-[0.2em] mb-1.5 font-bold">بيانات الجهاز</div>
                          <div className="space-y-1">
                            {[{k:'الشركة',v:dev.manufacturer},{k:'الطراز',v:dev.model},{k:'الرقم',v:dev.serial},{k:'الإصدار',v:dev.firmware},{k:'قوة الإرسال',v:dev.txPower!=null?`${dev.txPower} dBm`:undefined}].filter(x=>x.v).map(x=>(
                            ...
                            {dev.geo&&<div className="flex gap-2"><span className="text-[9px] font-mono text-[var(--text-muted)] w-[65px] shrink-0 tracking-wider">الموقع</span><span className="text-[9px] font-mono text-[#FFB74D]">{dev.geo.lat.toFixed(5)}, {dev.geo.lng.toFixed(5)} ±{Math.round(dev.geo.acc)}م</span></div>}
                        ...
                        <div className="flex gap-1.5 flex-wrap">
                          {dev.bt&&(dev.connected?(<>
                            <motion.button whileTap={{scale:0.95}} onClick={()=>explore(dev)} className="flex-1 py-1.5 rounded-lg text-[9px] font-mono font-bold tracking-wider flex items-center justify-center gap-1.5" style={{background:`${dev.color}08`,color:dev.color,border:`1px solid ${dev.color}10`}}><Layers className="w-3 h-3"/>خدمات GATT</motion.button>
                            <motion.button whileTap={{scale:0.95}} onClick={()=>disconnect(dev)} className="py-1.5 px-2.5 rounded-lg text-[9px] font-mono font-bold flex items-center gap-1" style={{background:'rgba(255,61,61,0.05)',color:'#FF6B6B',border:'1px solid rgba(255,61,61,0.06)'}}><Unplug className="w-3 h-3"/>قطع</motion.button>
                          </>):(<>
                            <motion.button whileTap={{scale:0.95}} onClick={()=>connect(dev)} disabled={!!connecting} className="flex-1 py-2 rounded-lg text-[9px] font-mono font-bold tracking-wider flex items-center justify-center gap-1.5 disabled:opacity-30" style={{background:`${dev.color}08`,color:dev.color,border:`1px solid ${dev.color}10`}}><Zap className="w-3 h-3"/>{isC?'جارٍ الاقتران...':'اتصال'}</motion.button>
                          </>))}
                          {dev.geo&&<motion.button whileTap={{scale:0.95}} onClick={()=>placeSingleOnMap(dev)} className="py-1.5 px-2.5 rounded-lg text-[9px] font-mono font-bold tracking-wider flex items-center gap-1" style={{background:'rgba(255,183,77,0.06)',color:'#FFB74D',border:'1px solid rgba(255,183,77,0.08)'}}><MapPin className="w-3 h-3"/>عرض على الخريطة</motion.button>}
                        </div>
                        ...
                        <AnimatePresence>{isG&&(<motion.div initial={{height:0,opacity:0}} animate={{height:'auto',opacity:1}} exit={{height:0,opacity:0}} className="overflow-hidden"><div className="mt-2.5 pt-2.5" style={{borderTop:`1px solid ${dev.color}08`}}>
                          <div className="flex items-center justify-between mb-2"><span className="text-[9px] font-mono font-bold tracking-[0.12em]" style={{color:dev.color}}>شجرة GATT</span><button onClick={()=>{setGattTarget(null);setGattSvcs([]);}}><X className="w-2.5 h-2.5 text-[var(--text-muted)]"/></button></div>
                          {gattLoading?<div className="flex items-center justify-center py-3 gap-1.5"><div className="w-3.5 h-3.5 border-2 rounded-full animate-spin" style={{borderColor:dev.color,borderTopColor:'transparent'}}/><span className="text-[9px] font-mono animate-pulse" style={{color:dev.color}}>جارٍ الاستعراض...</span></div>
              ...
              <div className="flex-1 overflow-y-auto styled-scrollbar p-2.5 space-y-2.5">
                ...
                <div className="rounded-xl overflow-hidden" style={{background:'rgba(255,255,255,0.015)',border:'1px solid rgba(255,255,255,0.04)'}}>
                  <div className="px-3 py-2 flex items-center justify-between" style={{borderBottom:'1px solid rgba(255,255,255,0.03)'}}>
                    <div className="flex items-center gap-1.5"><Globe className="w-3 h-3 text-[#80DEEA]"/><span className="text-[9px] font-mono font-bold tracking-[0.12em] text-[var(--text-primary)]">فحص الشبكة</span></div>
                    <motion.button whileTap={{scale:0.95}} onClick={runNetRecon} disabled={netLoading} className="px-2 py-0.5 rounded text-[9px] font-mono font-bold tracking-wider disabled:opacity-30" style={{background:'rgba(128,222,234,0.06)',color:'#80DEEA',border:'1px solid rgba(128,222,234,0.08)'}}>{netLoading?'...':'فحص'}</motion.button>
                  </div>
                  {!netIntel&&<div className="px-3 py-4 text-center"><span className="text-[9px] font-mono text-[var(--text-muted)] opacity-30">اضغط «فحص» للتحقق من الشبكة المحلية</span></div>}
                  {netIntel&&<div className="p-2.5 space-y-2">
                    {netIntel.localIPs.length>0&&<div><div className="text-[9px] font-mono text-[var(--text-muted)] tracking-wider mb-1 font-bold">عناوين IP المحلية</div>{netIntel.localIPs.map((ip,i)=><div key={i} className="text-[10px] font-mono text-[#80DEEA] pl-2">{ip}</div>)}</div>}
                    {netIntel.type&&<div className="flex gap-3">{[{l:'النوع',v:netIntel.type},{l:'التنزيل',v:netIntel.downlink!=null?`${netIntel.downlink}Mbps`:undefined},{l:'RTT',v:netIntel.rtt!=null?`${netIntel.rtt}ms`:undefined}].filter(x=>x.v).map(x=><div key={x.l}><div className="text-[9px] font-mono text-[var(--text-muted)] tracking-wider">{x.l}</div><div className="text-[10px] font-mono text-[var(--text-primary)] font-bold">{x.v}</div></div>)}</div>}
                    {netIntel.openPorts.length>0&&<div><div className="text-[9px] font-mono text-[var(--text-muted)] tracking-wider mb-1 font-bold">المنافذ المفتوحة</div>{netIntel.openPorts.map((p,i)=><div key={i} className="flex items-center gap-2 pl-2 py-0.5"><span className="text-[9px] font-mono font-bold text-[var(--alert-green)]">:{p.port}</span><span className="text-[9px] font-mono text-[var(--text-muted)]">{p.service}</span></div>)}</div>}
                  </div>}
                </div>
                {/* Export */}
                <div className="rounded-xl overflow-hidden" style={{background:'rgba(255,255,255,0.015)',border:'1px solid rgba(255,255,255,0.04)'}}>
                  <div className="px-3 py-2 flex items-center gap-1.5" style={{borderBottom:'1px solid rgba(255,255,255,0.03)'}}><Database className="w-3 h-3 text-[#FFB800]"/><span className="text-[9px] font-mono font-bold tracking-[0.12em] text-[var(--text-primary)]">الحفظ والتصدير</span></div>
              ...
              <div className="flex-1 flex flex-col overflow-hidden">
                <div className="flex items-center justify-between px-3 py-1 shrink-0" style={{background:'rgba(0,0,0,0.3)',borderBottom:'1px solid rgba(255,255,255,0.03)'}}>
                  ...
                  <div className="flex items-center gap-1.5">
                    <button onClick={()=>setPaused(!paused)} className={`text-[9px] font-mono tracking-wider px-1 py-0.5 rounded flex items-center gap-0.5 ${paused?'text-[#FFB800]':'text-[var(--text-muted)]'}`}>{paused?<><Pause className="w-2 h-2"/>متوقف مؤقتًا</>:<><Play className="w-2 h-2"/>مباشر</>}</button>
                    <button onClick={()=>{setPkts([]);setBytes(0);pid.current=0;}} className="text-[9px] font-mono text-[var(--text-muted)] tracking-wider px-1 py-0.5 rounded hover:bg-white/5 flex items-center gap-0.5"><Trash2 className="w-2 h-2"/>مسح</button>
                  </div>
                </div>
                <div ref={logEl} className="flex-1 overflow-y-auto styled-scrollbar font-mono" style={{background:'#020406'}}>
                  {pkts.length===0?<div className="flex items-center justify-center h-full opacity-15"><span className="text-[9px] font-mono text-[var(--text-muted)] tracking-wider">ابدأ الفحص لعرض السجل...</span></div>
                  :<div className="py-0.5">{pkts.map(p=>{const oc=OP_CLR[p.op]||'#90A4AE';return(<div key={p.id} className="px-2 py-[2px] flex items-start hover:bg-white/[0.015] border-b border-white/[0.01] group" style={{fontSize:10,lineHeight:'16px'}}><span className="text-white/8 shrink-0 w-[24px] tabular-nums text-right pr-1.5">{p.id}</span><span className="text-[var(--text-muted)]/30 shrink-0 w-[58px] tabular-nums">{fts(p.ts)}</span><span className="shrink-0 font-bold tracking-wider w-[58px]" style={{color:oc}}>{OP_LABEL[p.op]||p.op}</span><span className="text-[var(--cyan-primary)]/40 shrink-0 truncate w-[60px]">{SRC_LABEL[p.src]||p.src}</span><span className="text-[var(--text-secondary)] flex-1 truncate">{p.msg}</span>{p.len!=null&&<span className="text-[var(--text-muted)]/12 ml-1 shrink-0 tabular-nums">{p.len}B</span>}</div>);})}</div>}
                </div>
              </div>
            )}
          </div>
          ...
          <div className="flex items-center justify-between px-3 py-1.5 shrink-0" style={{borderTop:'1px solid rgba(255,255,255,0.03)',background:'rgba(0,0,0,0.2)'}}>
            <div className="flex items-center gap-1.5"><div className={`w-1.5 h-1.5 rounded-full ${btOk?'bg-[var(--alert-green)]':'bg-[#FF3D3D]'} ${scanning?'animate-pulse':''}`}/><span className="text-[9px] font-mono text-[var(--text-muted)] tracking-wider">{scanning?'جارٍ الفحص':liveCount>0?`${liveCount} مباشر`:'جاهز'} · {upStr}</span></div>
            <span className="text-[9px] font-mono text-[var(--text-muted)]/15 tracking-[0.12em]">M3TM.WORLD V8</span>
          </div>
        </motion.div>
      )}</AnimatePresence>
    </div>
