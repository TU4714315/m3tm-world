...
export default function MapControls({ mapRef, onInteract }: MapControlsProps) {
  ...
  return (
    <motion.div initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }}
      ...
      <div role="group" aria-label="أدوات تحريك وتكبير الخريطة"
        ...
        <div className="flex flex-col gap-[3px]">
          <Btn label="تكبير" move={{ kind: 'zoom', dir: 1 }} icon={Plus} disabled={limits.max} press={press} release={release} step={step} />
          <Btn label="تصغير" move={{ kind: 'zoom', dir: -1 }} icon={Minus} disabled={limits.min} press={press} release={release} step={step} />
        </div>
        <div className="desktop-only flex items-center gap-1">
          ...
          <div className="grid grid-cols-3 grid-rows-3 gap-[3px]">
            <Btn cell="col-start-2 row-start-1" label="تحريك شمالًا" move={{ kind: 'pan', dx: 0, dy: -1 }} icon={ChevronUp} press={press} release={release} step={step} />
            <Btn cell="col-start-1 row-start-2" label="تحريك غربًا" move={{ kind: 'pan', dx: -1, dy: 0 }} icon={ChevronLeft} press={press} release={release} step={step} />
            <Btn cell="col-start-3 row-start-2" label="تحريك شرقًا" move={{ kind: 'pan', dx: 1, dy: 0 }} icon={ChevronRight} press={press} release={release} step={step} />
            <Btn cell="col-start-2 row-start-3" label="تحريك جنوبًا" move={{ kind: 'pan', dx: 0, dy: 1 }} icon={ChevronDown} press={press} release={release} step={step} />
          </div>
        </div>
      </div>
    </motion.div>
...
function Btn({ cell = '', label, icon: Icon, move, disabled = false, press, release, step }: {
  ...
  return (
    <button type="button" title={`${label} — اضغط مطولًا للاستمرار`} aria-label={label} disabled={disabled}
